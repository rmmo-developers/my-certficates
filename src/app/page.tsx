...
function VerifyContent() {
  ...
  const processDecodedText = (decodedText: string) => {
    ...
    if (decodedText.startsWith("http")) {
      try {
        ...
        extractedId = queryParam || url.pathname.split('/').pop() || decodedText;
      } catch (e) { extractedId = decodedText; }
    }
    const cleanId = extractedId.toUpperCase().startsWith("RMMO-") ? extractedId.toUpperCase() : `RMMO-${extractedId.toUpperCase()}`;
    setCertId(cleanId);
    autoVerify(cleanId);
  ...
  const autoVerify = async (id: string) => {
    ...
    if (res.success) {
      ...
      window.history.replaceState(null, "", `${window.location.pathname}?c=${idOnly}`);
      setResult(res.data);
      setViewSummary(true);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.startsWith("RMMO-RMMO-")) value = value.replace("RMMO-RMMO-", "RMMO-");
      ...
      <div className="min-h-screen bg-[#F8F9FF] text-slate-900 font-sans flex flex-col">
        <main className="flex-1 flex flex-col items-center justify-center px-6 pb-24">
          ...
          <div className="w-full max-sm:max-w-xs md:max-w-sm">
            ...
            {isScannerOpen && (
              <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex flex-col items-center justify-center p-6">
                <div id="reader" className="w-full max-w-md bg-white rounded-[28px] overflow-hidden shadow-2xl"></div>
                <button onClick={() => setIsScannerOpen(false)} className="mt-8 px-8 py-3 bg-blue-700 text-white font-medium rounded-full hover:bg-blue-800 transition">Close Scanner</button>
              </div>
            )}
          </div>
        </main>
        ...
        <footer className="w-full max-w-xl mx-auto px-6 pb-10 text-center space-y-4">
            <p className="text-[13px] text-slate-500 leading-relaxed">
                Your Certificate Number is located below the certificate together with the QR Code.
            </p>
            <p className="text-[11px] text-slate-400 leading-relaxed italic">
             By granting camera permissions the QR code scanner scans locally without uploading any data or stored in our servers, ensuring that your information stays private.
            </p>
  ...
  return (
    <div className="min-h-screen lg:h-screen bg-[#F8F9FF] text-slate-900 font-sans flex flex-col overflow-y-auto lg:overflow-hidden">
      <nav className="max-w-7xl mx-auto w-full px-4 md:px-8 py-4 flex justify-between items-center shrink-0">
        <button onClick={resetSearch} className="cursor-pointer flex items-center gap-2 text-blue-700 hover:bg-blue-50 px-4 py-2 rounded-full font-medium text-[14px] transition">
      ...
      <main className="max-w-7xl mx-auto w-full px-4 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 lg:min-h-0 pb-6">
        ...
        <div className="lg:col-span-4 space-y-4 lg:overflow-y-auto lg:pr-2">
          <div className="bg-white p-6 rounded-[28px] border border-slate-100 shadow-sm flex flex-col items-center">
            ...
            <div className="flex flex-col items-center mb-6">
                <CheckCircleIcon />
                <span className="mt-2 text-[12px] font-bold text-blue-700 uppercase tracking-[0.2em]">Document Found</span>
            </div>
            
            <div className="w-full space-y-6">
              <section>
                <p className="text-[11px] font-bold text-slate-400 uppercase mb-1">Issued to</p>
                <p className="text-[14px] font-medium text-slate-700 uppercase">{result.issued_to}</p>
              </section>

              <section>
                <p className="text-[11px] font-bold text-slate-400 uppercase mb-1">Certificate Number</p>
                <p className="font-mono font-bold text-[14px] text-blue-700">{result.cert_number}</p>
              </section>

              <section>
				  <p className="text-[11px] font-bold text-slate-400 uppercase mb-1">Validity</p>
				  <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg text-[12px] font-bold ${
              ...
              <div className="pt-4 border-t border-slate-50 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-[11px] font-bold text-slate-400 uppercase">Document Type</span>
                  <span className="text-[14px] font-medium text-slate-700">{result.type}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[11px] font-bold text-slate-400 uppercase">Term</span>
                  <span className="text-[14px] font-medium text-slate-700">{result.school_year}</span>
                </div>
				  <div className="flex justify-between items-center">
          ...
          <div className="bg-blue-700 p-5 rounded-[24px] text-white shadow-lg shadow-blue-100">
            <p className="text-[12px] font-medium leading-relaxed opacity-90 text-center lg:text-left">
              This record is officially recognized by the institution. Any unauthorized modifications are strictly prohibited.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 flex flex-col min-h-[500px] lg:min-h-0">
          <div className="bg-white p-4 rounded-[28px] shadow-sm border border-slate-100 flex-1 flex flex-col min-h-0">
            <div 
                onClick={() => !imageError && result.google_photos_link && setIsFullscreen(true)}
            ...
            >
              {!imageError && result.google_photos_link ? (
                ...
                <div className="flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <h4 className="text-slate-900 font-medium mb-1 text-[14px]">Preview not available</h4>
                  <p className="text-slate-400 text-[12px] max-w-xs leading-relaxed">Source image is currently processing.</p>
                </div>
              )}
            </div>
            <div className="mt-3 px-2 flex justify-between items-center shrink-0">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Digital Hash: {result.cert_number?.split('-')[1]}</span>
            </div>
          </div>
        </div>
      </main>
      ...
      {isFullscreen && !imageError && (
        <div className="fixed inset-0 z-[300] bg-black/95 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            <img src={getImageUrl(result.google_photos_link)} className="max-w-full max-h-full object-contain shadow-2xl" alt="Full View" />
        </div>
      )}
    </div>
  );
